<template>
  <div class="pay_method__paymethod p-3 p-md-4 p-lg-6 p2-bg rounded-8">
    <div class="pay_method__paymethod-title mb-5 mb-md-6">
      <h2 class="text-white fw-bold mb-2">{{ $t('ui.deposit_management') }}</h2>
      <p class="text-white-50">{{ $t('ui.manage_all_platform_deposits') }}</p>
    </div>
        ...
        <table class="w-100 text-center p2-bg">
          ...
          <tbody>
            ...
            <tr v-if="filteredDeposits.length === 0">
              <td colspan="7" class="text-white p-5 text-center">{{ $t('ui.no_deposits') }}</td>
            </tr>
          </tbody>
        </table>
      </div>
